...
1. **Le Système de Rating (Difficulté) :** * Le score d'un mot n'est pas aléatoire. Il est calculé selon la fréquence d'apparition des lettres en français (Wikipedia). Plus un mot contient de lettres rares (X, Z, W, K), plus sa note est élevée.
   * La formule prend aussi en compte le ratio lettres uniques / longueur totale pour éviter les mots trop simples.

2. **Mode Mort Subite :** * Une mécanique de jeu où le joueur enchaîne les mots.
...
3. **Interface Graphique :** * Utilisation de la bibliothèque `CustomTkinter` pour un rendu moderne.
